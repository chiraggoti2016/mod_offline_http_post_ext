...
-author("gotichirag2011@gmail.com").
...
ccreate_message({Action, Packet} = Acc) when (Packet#message.type == chat) and (Packet#message.body /= []) ->
  ...
  post_offline_message(ToUser, FromUser, PostUrl, Token, Body),
  Acc;

ccreate_message(Acc) ->
  Acc.

post_offline_message(ToUser, FromUser, PostUrl, Token, Data) ->
  ?INFO_MSG("post ~p to to ~p using ~p~n ", [Data, PostUrl, Token]),
  Body = string:join(["to=", binary_to_list(ToUser), "&from=", binary_to_list(FromUser), "&content=", binary_to_list(Data)], ""),
  Request = {binary_to_list(PostUrl), [{"Authorization", binary_to_list(Token)}], "application/x-www-form-urlencoded;  charset=utf-8", Body},
